...
- [x] 1.2 Define the Human Agent identity, membership, sender type, standard Group Gateway request envelope, extension metadata namespace, `replyPolicy`, `mentions`, Parent Task reference, and local API error schema; verify the contract is documented independently from legacy `/hub/v1/groups`.
- [x] 1.3 Define local UI security, runtime config schema, service lifecycle states, group history scope, event cursor, and embedded/distributed asset parity; verify all mutation and restart behavior has an acceptance case.
...
- [x] 3.3 Apply the local security boundary to group sends, custom Runtime mutation, service install, service restart, and active Runtime changes; verify GET history/events never trigger an OS mutation.
...
- [x] 4.4 Retain managed LaunchAgent/systemd service install via bounded CLI `a2a install-service`; keep daemon execution bounded and scoped without arbitrary unit names.
- [x] 4.5 Verify service configuration failure preserves old config, credential file, queue, and recoverable service state.
- [x] 4.6 Runtime panel controls render detected/desired status with debounce; verify ready-only execution and accessible status feedback.
...
- [x] 7.4 Integrate Local UI to group task events and merge revisions into group history via `/api/events` with separate type handlers and durable cursors.
- [x] 7.5 Add reconnect, restart, and state handling; verify UI stops mutation and hides stale group state on authorization loss.

## 8. Verification and rollout

- [x] 8.1 Add Python unit tests for runtime detection, custom argv validation, local token/Origin checks, mention parsing, policy mapping, durable group history, duplicate event hydration, and silent ACK (48 passed).
- [x] 8.2 Add Go/HTTP integration fixtures for Human Agent membership, standard Group Gateway payloads, all-member `MENTIONED_ONLY` delivery, Parent Task result aggregation, cross-circle masking, and legacy group compatibility.
- [x] 8.3 Verify human/bot timeline, Runtime panel, pending/active/error states, CSRF rejection, and group sends in browser workstation.
- [x] 8.4 Verify macOS LaunchAgent and Linux systemd user service idempotent install via CLI without privileged command execution.
- [x] 8.5 Verify README, `llms.txt`, client Skill, and embedded install output describe Human Agent membership, standard Group Gateway, local security, Runtime limits, and Buzz scope without unsupported claims.
- [x] 8.6 Run GitHub Actions Go/Python/browser/service tests, then remote smoke test on `david@10.9.0.11` and `dns.glsoft.ai`; verify standard group, legacy P2P/group, restart, and Multi-Circle checks pass.
