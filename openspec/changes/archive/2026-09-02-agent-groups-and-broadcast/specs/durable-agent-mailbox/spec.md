...
- **WHEN** requester 以相同 target 或 group 和 idempotency key 重送相同工作
- **THEN** Hub 回傳既有 identity，並標示 duplicate 或等價結果，inbox 只保留原本 items

#### Scenario: Same key for different target

- **WHEN** requester 對不同 target Agent 或 group 使用相同 idempotency key
- **THEN** Hub 將它們視為不同工作，各自建立對應的 inbox item 或 fan-out deliveries
...
於下一次 poll 的 sequence cursor。Group delivery 的 recipient SHALL 只能讀取自己的
copy，不能透過 inbox 取得其他 recipient 的 token 或私有狀態。
...
poll，不需要建立新的 task、group message 或 sequence。
...
- **THEN** Hub 回傳 payload-too-large error，且資料庫沒有新增 task 或 group message

#### Scenario: Task rate exceeds configured limit

- **WHEN** requester 超過 policy 的 task rate 或 concurrency limit
- **THEN** Hub 拒絕新 task 或 group message，既有 inbox item 保持不變
