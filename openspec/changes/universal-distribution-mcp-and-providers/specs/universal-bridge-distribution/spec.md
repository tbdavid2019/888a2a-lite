...
- **WHEN** user executes `curl -fsSL https://<hub>/install.sh | bash -s -- --name "甘露寺" --backend openclaw` on a Linux or macOS host with Python 3.8+
...
- **WHEN** user executes `npm install -g 888a2a` or `npx 888a2a`
